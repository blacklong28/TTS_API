...
static void end_sr_on_vad(struct speech_rec *sr)
{
	...
	while(sr->rec_stat != MSP_REC_STATUS_COMPLETE ){
		...
		if(rslt!=NULL){
			strcat(sr->notif.result,rslt);
			//memcpy(sr->notif.result,rslt,strlen(rslt)*sizeof(char));
		}
		if (rslt && sr->notif.on_result)
			sr->notif.on_result(rslt, sr->rec_stat == MSP_REC_STATUS_COMPLETE ? 1 : 0);

		//Sleep(3); /* for cpu occupy, should sleep here */
	}
...
int sr_start_listening(struct speech_rec *sr)
{
	...
	if(byd_session_id_isr!=NULL){
		printf("session_id!=NULL\n");
		QISRSessionEnd(byd_session_id_isr, "byd_exit2");
	}
	if(sr->session_id!=NULL){
		printf("sr->session_id!=NULL\n");
		QISRSessionEnd(sr->session_id, "byd_exit2");
	}
begin:
	session_id = QISRSessionBegin(NULL, sr->session_begin_params, &errcode); //��д����Ҫ�﷨����һ������ΪNULL
	if (MSP_SUCCESS != errcode)
	{
		sr_dbg("\nQISRSessionBegin failed! byd  error code:%d\n", errcode);
		/*
		if(errcode==10132){
			if(api_id==1){
	...
	//session_id_byd = session_id;
	//api_id = 2;
	sr->session_id = session_id;
	byd_session_id_isr = session_id;
	sr->ep_stat = MSP_EP_LOOKING_FOR_SPEECH;
	sr->rec_stat = MSP_REC_STATUS_SUCCESS;
	sr->audio_status = MSP_AUDIO_SAMPLE_FIRST;
